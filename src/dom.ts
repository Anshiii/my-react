...
/* 创建的时候只创建父元素？ */
export function createDomElement(ele:element){
    let domEle ;
    switch(typeof ele.type){
        case TEXT_ELEMENT:
        domEle = document.createTextNode(ele.props.nodeValue)
        case 'string':
        case 'function':
        case 'object':
         domEle = document.createElement(ele.type);
    }
    /* 创建 dom 元素时还需要设置 prop */
    updateDomProperties(domEle,{},ele.props);
    return domEle
}